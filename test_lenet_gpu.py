...
for epoch in range(1):
    ...
    for i in range(images.shape[0] // batch_size):
        ...
        conv1_out = relu1.forward(conv1.forward_gpu(img))
        pool1_out = pool1.forward_gpu(conv1_out)
        #conv2_out = relu2.forward(conv2.forward(pool1_out))
        conv2_out = relu2.forward(conv2.forward_gpu(pool1_out))
        pool2_out = pool2.forward_gpu(conv2_out)
